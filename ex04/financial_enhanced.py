...
def main(short_name,name_row):
    ...
    req = http.request("GET",url,headers=headrs)
...
# python3 -m cProfile -s cumulative financial_enhanced.py 'MSFT' 'Total Revenue' > profiling-http.txt
# python3 -m cProfile -s calls financial_enhanced.py 'MSFT' 'Total Revenue' > profiling-ncalls.txt

# python -m cProfile -o profile_results.prof financial_enhanced.py 'MSFT' 'Total Revenue'
